...
PurePursuitParams PurePursuitController::getDefaultParams()
{
    ...
    params.goal_tolerance = 0.015f; // ✅ RELAXED: 15mm instead of 1mm
    ...
    params.max_distance_error = 2.0f; // ✅ RELAXED: 2m for multi-waypoint paths
    params.timeout_ms = 60000;        // 60s timeout
    params.enable_collision_check = false;

    // Performance defaults
    params.enable_fast_math = false; // ✅ FIXED: changed from float to bool

    return params;
}

// ✅ FIXED: Main update function - separated from setTarget
RobotVelocity PurePursuitController::update(const Pose2D &current_pose)
{
    ...
    {
        // THÊM DEBUG
        Serial.print("PP DEBUG: Distance=");
        Serial.print(_distance_to_target, 4);
        Serial.print("m, Tolerance=");
        Serial.print(_params.goal_tolerance, 4);
        Serial.print("m, Pose=(");
        Serial.print(current_pose.x, 3);
        Serial.print(",");
        Serial.print(current_pose.y, 3);
        Serial.print("), Target=(");
    ...
    const Pose2D &current_pose, float dx, float dy, float distance)
{
    ...
    // Dead zone for small heading errors (0.01 degree = 0.0001745 rad)
    const float HEADING_DEADZONE = 0.05f; // 0.05 rad ~ 2.8 degrees
    if (abs(alpha) < HEADING_DEADZONE)
    {
        alpha = 0.0f;
        _sin_alpha = 0.0f;
        _cos_alpha = 1.0f;
    ...
    {
        // Cache sin/cos if angle hasn't changed much
...
// ✅ IMPROVED: Safety condition checks
bool PurePursuitController::checkSafetyConditions(const Pose2D &current_pose)
{
    ...
    // Distance error check - only when actively following
    ...
    // ✅ IMPROVED: Check distance from start (only if enabled)
    if (_max_distance_from_start > 0.001f)
    {
        ...
        if (dist_from_start > _max_distance_from_start * 3.0f)
        {
            _state = PP_SAFETY_STOP;
...
void PurePursuitController::setTarget(const TargetPoint &target)
{
    ...
    // ✅ IMPROVED: Calculate realistic max distance for safety check
    // For single waypoint: use distance to target
    // For multi-waypoint: this will be overridden by user
